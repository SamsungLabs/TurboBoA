...
class MinMaxQuantizer(nn.Module):
    ...
    def find_params_H(self, weight, H_in, search):
        w_min, w_max = get_min_max_range(weight, self.sym)
        scale, zero = grid_search(weight, w_max, w_min, self.maxq.to(weight.device), self.sym, self.norm, self.grid, H_in, search)
            
        return scale, zero
    

    def find_params_H_group(self, weight, H_in, search):
        '''
            weight.shape = [..., n_groups, group_size]
                - for channel_wise quantization, n_groups=1, group_size=hidden_size
            H_in.shape = [..., hidden_size, hidden_size]
        '''
        n_groups, group_size = weight.shape[-2:]
        weight = weight.transpose(-2, -3)
        if H_in is not None:
            # take block diagonals each of which corresponds to one group.
            H_in = torch.stack(
                [H_in[..., i*group_size:(i+1)*group_size, i*group_size:(i+1)*group_size] for i in range(n_groups)], dim=-3
            )

        w_min, w_max = get_min_max_range(weight, self.sym)
        scale, zero = grid_search(weight, w_max, w_min, self.maxq, self.sym, self.norm, self.grid, H_in, search)

        # The shape of obtained scale and zero is [..., n_groups, d_out, 1]
        scale = scale.transpose(-2, -3)
        zero = zero.transpose(-2, -3)
        
        self.scale = scale.reshape(self.scale.shape)
        self.zero = zero.reshape(self.zero.shape)
